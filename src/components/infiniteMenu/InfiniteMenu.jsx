...
import styles from './InfiniteMenu.module.scss';
...
class InfiniteGridMenu {
    ...
    #render() {
        ...
        if (theme === 'dark') {
            gl.clearColor(0.0, 0.0, 0.0, 1.0); // Black for dark theme
        } else {
            gl.clearColor(227 / 255, 236 / 255, 255 / 255, 1.0); // White for light theme
        }
...
export default function InfiniteMenu({ items = [], theme = 'light' }) {
    ...
    return (
        ...
        >
            <canvas className={cx('infinite-grid-menu-canvas')} ref={canvasRef} />
            ...
            {activeItem && (
                <>
                    ...
                    <div onClick={handleButtonClick} className={cx('action-button', isMoving ? 'inactive' : 'active')}>
                        <p className={cx('action-button-icon')}>&#x2197;</p>
                    </div>
                </>
            )}
        </div>
